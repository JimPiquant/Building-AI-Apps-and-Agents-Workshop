"""
Day 4 Demo — graph plumbing for Module 3's visualization demo.

Copied from labs/day4/python/workflow_nodes.py, WITH the Part B2 guardrail
fix already applied (see RevisionGate.decide below) — this file is used
for a live demo, not as a student exercise, so it needs to be safe to run
without an unbounded loop. If you're looking for the student-facing
version with the TODO left in, see labs/day4/python/workflow_nodes.py and
labs/day4/python/solutions/part_b2_revision_gate.py.
...
READING ORDER
  ...
  4. `RevisionGate` ....... Critic -> loop back, or finish (bounded)
...
class RevisionGate(Executor):
    ...
    Bounded here (Part B2's completed fix, applied for this demo) —
    approves immediately when the Critic approves, and gives up
    gracefully at MAX_REVISIONS otherwise instead of looping forever.
    """
